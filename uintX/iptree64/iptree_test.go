...
func assertTree32Node(r *Tree, key uint32, bits int, e uint64, desc string, t *testing.T) {
	v, ok := r.root32.ExactMatch(key, bits)
	assertResult(v, ok, e, fmt.Sprintf("0x%08x, %d for %s", key, bits, desc), t)
}

func assertTree64Node(r *Tree, MSKey uint64, MSBits int, LSKey uint64, LSBits int, e uint64, desc string, t *testing.T) {
...
func assertResult(v uint64, ok bool, e uint64, desc string, t *testing.T) {
	if ok {
		if v != e {
